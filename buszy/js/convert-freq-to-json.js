...
let departureTimesHeaders = [];

// Detect format: check for "Departure Times" or "Weekdays", "Saturdays" as column headers
if (lines.some(line => line.includes('Departure Times') || line.includes('Departure times'))) {
    isDepartureTimes = true;
}

// Check for day types as column headers (departure times format)
if (!isDepartureTimes) {
    for (let i = 0; i < Math.min(3, lines.length); i++) {
        const parts = lines[i].split('\t').map(p => p.trim());
        if (parts.length >= 2 && parts.some(p => dayTypeMap[p])) {
            isDepartureTimes = parts.every(p => !p.includes('6:30am') && !p.includes('8.31am') && !p.includes('5.00pm'));
            if (isDepartureTimes) {
                departureTimesHeaders = parts;
                break;
            }
        }
    }
...
// First pass: check if file has "From" headers (multi-direction format for frequencies)
hasDirections = lines.some(line => line.startsWith('From')) && !isDepartureTimes;

if (isDepartureTimes) {
    // Parse departure times format
    let headerLineIndex = -1;
    let dayTypeIndices = {};
    
    // Find header line with day types
    for (let i = 0; i < lines.length; i++) {
        const parts = lines[i].split('\t').map(p => p.trim());
        if (parts.some(p => dayTypeMap[p])) {
            headerLineIndex = i;
            // Map day types to their column indices
            parts.forEach((p, idx) => {
                if (dayTypeMap[p]) {
                    dayTypeIndices[dayTypeMap[p]] = idx;
                }
            });
            break;
        }
    ...
    if (headerLineIndex >= 0) {
        for (let i = headerLineIndex + 1; i < lines.length; i++) {
            ...
            // Get the time period or description (first column or use previous row)
            const rowLabel = parts[0];
            
            // Create entry for this row
            departure_times[rowLabel] = {};
            
            // Extract times for each day type
            Object.entries(dayTypeIndices).forEach(([dayType, idx]) => {
                if (parts[idx]) {
                    departure_times[rowLabel][dayType] = parts[idx];
                }
            });
        }
    }
} else {
    // Original frequency parsing logic
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
...
// Generate output based on format detected
let output = {};

if (isDepartureTimes) {
    output = { "departure_times": departure_times };
